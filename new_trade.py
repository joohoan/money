...
df_btc = pyupbit.get_ohlcv("KRW-BTC", interval="day", count=1)
y_btc = df_btc.iloc[0]['open']
...
while True:
    try:
        ...
        if current_price_btc - y_btc > 0:
            print("매수를 명령합니다.")
            for i in range(30) :
                target_price = get_target_price(interest[i], 0.4)
                current_price = get_current_price(interest[i])
                upper_price = target_price * 1.005
                lower_price = target_price * 0.99
                ikjul_price = target_price * 1.025
                print(interest[i])
                print(target_price)
